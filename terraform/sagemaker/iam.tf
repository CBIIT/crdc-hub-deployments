#create role for sagemaker instance notbook
...
# create role for sagemaker studio
...
# attach AWS default policy to the role sagemaker studio role
resource "aws_iam_role_policy_attachment" "sagemaker_studio_full_access" {
  role       = aws_iam_role.sagemaker_studio_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSageMakerFullAccess"
}

#attach AWS bedrock full access to the sagemake studio role
